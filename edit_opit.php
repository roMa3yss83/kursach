<?php
session_start();
$site_title = 'Редактирование опыта работы';
...
$selectwork = $db->prepare("SELECT * FROM `work_experience_user` WHERE `id` = ?");
$selectwork->execute(array($_GET['opit']));
$selectwork = $selectwork->fetch(PDO::FETCH_ASSOC);

if((isset($_POST["text"])) and (isset($_POST["place"]))) {
    if((isset($_COOKIE['user'])) and ($_COOKIE['user'] == $selectwork['id_user'])) {
        $work = $db->prepare("UPDATE `work_experience_user` SET 
        `text` = ?, `duration_work` = ?, `place` = ? WHERE `id` = ?");
        $work = $work->execute(array($_POST['text'], 
            $_POST['duration_work'], $_POST['place'], $_GET['opit']));
        
        $_SESSION['success'] = 'Успешно была обновлена информация о предыдущем опыте работы!';
    ...
    <main>
      <div class="container lox">
        ...
        <form action="" class="w-50" method="POST">
            ...
            <input value="<?php echo $selectwork['place']; ?>" type="text" name="place" 
            class="form-control" placeholder="ПАО КАМАЗ" />
            <b>Сколько месяцев работали (напишите в цифрах)</h3>
            <input value="<?php echo $selectwork['duration_work']; ?>" type="text" name="duration_work" 
            ...
            <textarea name="text" id="editor"><?php echo $selectwork['text']; ?></textarea><br>
